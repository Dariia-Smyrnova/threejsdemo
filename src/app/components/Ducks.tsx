...
function RubberDuck({ index, z, speed }) {
  const ref = useRef()
...
export default function RubberDucks({ speed = 2, count = 200, depth = 80, easing = (x) => Math.sqrt(1 - Math.pow(x - 1, 2)) }) {
  return (
    ...
    <Canvas flat gl={{ antialias: false }} dpr={[1, 1.5]} camera={{ position: [0, 0, 10], fov: 20, near: 0.01, far: depth + 15 }}>
      ...
      {Array.from({ length: count }, (_, i) => <RubberDuck key={i} index={i} z={Math.round(easing(i / count) * depth)} speed={speed} /> /* prettier-ignore */)}
